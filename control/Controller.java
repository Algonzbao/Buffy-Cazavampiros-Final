...
public class Controller {
	...
	public void run() {
		...
		while (!game.isFinished()) {
			...
			String[] commandWords = s.toLowerCase().trim().split(" ");
			System.out.println("[DEBUG] Executing: " + s);
			Command command = CommandGenerator.parseCommand(commandWords, null);
			if (command != null) {
...
public class Controller {
	...
	public void run() {
		...
		while (game.vampiresLeft() && (!game.winVamp()) && !exit) {
			...
			String[] comand = new String[3];//el 3 es el limite max del comand
